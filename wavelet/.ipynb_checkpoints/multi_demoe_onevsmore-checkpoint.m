...
%% 	same  impulse response for each antenna
	...
	z_par = multi_fpar(wax, aximp, testyori, type, p,  noiseax, alpha_par);
	toc
	...
	% forward each 
		...
		z_single = multi_fone(wax(i,:), aximp(i,:), testyori, type, p, std(noiseax(i,:)), alpha_single);
		%subplot(5,3,i)
	...
	z_one = multi_fone(sum(wax)/15, sum(aximp)/15, testyori, type, p, std(sum(noiseax)/15), alpha_one);
